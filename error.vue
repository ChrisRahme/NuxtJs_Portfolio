<template>
  <div v-if="state['title']" id="error" class="noselect flex h-screen flex-col items-center justify-center">
    <h1 id="title" class="font-mono font-bold text-[#C06060]">
      {{ state['title'] }}
    </h1>

    <p id="message" class="font-mono text-[#C0C060]">
      {{ state['message'] }}
    </p>

    <!-- <NuxtLink to="/" id="button" class="btn w-52 p-4 flex justify-center items-center"> Go to Home </NuxtLink> -->
    <button id="button" class="btn flex w-52 items-center justify-center rounded-lg p-4" @click="goHome">Home</button>
  </div>
</template>

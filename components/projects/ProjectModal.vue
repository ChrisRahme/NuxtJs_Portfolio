        ...
        <Transition name="modal">
            ...
            >
                <div ref="panelRef" class="modal-panel" role="dialog" aria-modal="true" :aria-labelledby="titleId" tabindex="-1">
                    <button type="button" class="modal-close" @click="close" title="Close" aria-label="Close project details">
                        <Icon name="mdi:close" />
                    </button>
                    ...
                    <template v-if="props['project']">
                        <div class="modal-stage">
                            ...
                            <ModalYearRail :projects="props['projects']" :current="props['project']" @select="select" v-if="hasSiblings" />
                        </div>
                        ...
                        <div class="modal-paper">
                            ...
                            <nav class="modal-foot" aria-label="Older and newer projects" v-if="hasSiblings">
...
const newer = computed(function () {
    ...
    if (index < 0 || count < 2) return null
...
function getFocusable() {
    ...
    return Array.from(panelRef.value.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])')).filter(
...
watch(
    ...
    function () {
        nextTick(function () {
            ...
            const active = document.activeElement
            if (active && active.disabled && panelRef.value) panelRef.value.focus({ preventScroll: true })
        })
    }
)
...
.modal-stage {
    ...
    color: var(--star);
    background:
        radial-gradient(60% 50% at 50% 100%, rgba(201, 112, 92, 0.2) 0%, rgba(201, 112, 92, 0) 100%),

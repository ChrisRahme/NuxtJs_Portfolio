<template>
  <div class="skill-groups">
    <section v-for="group in groups" :key="group['title']" class="card skill-group" :style="{ '--theme': group['color'] }">
      <h3 class="eyebrow skill-group-title">{{ group['title'] }}</h3>

      <ul class="skill-list">
        <li v-for="item in group['icons']" :key="item['name']" class="skill-item" :style="{ '--color': item['color'] }">

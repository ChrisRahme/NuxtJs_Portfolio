<template>
  <article class="card exp-card" :style="{ '--theme': item['color'] || 'var(--green)' }">
    <header class="exp-head">
      ...
      <component :is="companyLink.tag" v-bind="companyLink.attrs" v-if="item['image']" class="exp-logo">
        <NuxtImg :src="item['image']" :alt="`${item['company']} logo`" width="56" height="56" format="webp" />
      </component>
    </header>

    <p v-if="item['description']" class="exp-desc" v-html="item['description']"></p>

    <ExperienceTaskList v-if="item['tasks']" :tasks="item['tasks']" />
  </article>
</template>

    ...
    <div class="wrap">
      ...
      <div class="work-rows">
        <ul v-for="(row, rowIndex) in rows" :key="rowIndex" class="work-row" :class="`cols-${row['items'].length}`">
          <li v-for="project in row['items']" :key="project['_id']" class="reveal">
            <NuxtLink
...
const ROW_PLANS: Record<number, number[]> = {
  1: [1],
  2: [1, 1],
  3: [1, 2],
  4: [1, 3],
...
function planRows(count: number) {
  const plan = ROW_PLANS[count]
  if (plan) {
    return plan
  }
...
const rows: { items: Project[]; mirror: boolean }[] = []
let cursor = 0

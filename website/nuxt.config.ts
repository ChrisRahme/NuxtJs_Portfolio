...
// Content Lake (see studio/). Project id and dataset are public; .env can override them.
const SANITY_PROJECT_ID = process.env.NUXT_SANITY_PROJECT_ID || '1ff3j3n2'
...
export default defineNuxtConfig({
  ...
  // Queries in sanity/ are auto-imported like composables
  imports: {
    dirs: ['sanity'],
  },

  // Content is fetched on each server render; the CDN is fine for a public dataset
  sanity: {
  ...
  // Own stylistic rules live in eslint.config.mjs;
  // Prettier owns formatting, so keep the module's stylistic off
  eslint: {
    config: {
      stylistic: false,
    },
